    ...
    <div className="space-y-6">
      ...
      <div id="advanced-simulation" className="glass-card p-6 space-y-5">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            <NumberInput label="Age" value={profile.age} min={18} max={90} unit="yrs"
              onChange={(v) => setProfile((p) => ({ ...p, age: v }))} />
            <NumberInput label="Weight" value={profile.weight_kg} min={30} max={250} unit="kg"
              onChange={(v) => setProfile((p) => ({ ...p, weight_kg: v }))} />
            <NumberInput label="Height" value={profile.height_cm} min={100} max={230} unit="cm"
      ...
      {result && (
        <div className="space-y-4 animate-fade-in">
          ...
          {(result.ai_summary || result.ai_action_plan) && (
            <div className="glass-card p-5 space-y-3 border border-brand-500/10">
              <h3 className="text-xs font-semibold text-brand-400 uppercase tracking-wider flex items-center gap-1.5">
                <span>🤖</span> AI Health Advisor (Powered by Groq)
              </h3>
              {result.ai_summary && (
                <div>
              ...
              {result.ai_action_plan && (
                <div>
                  <p className="text-[10px] text-slate-500 uppercase mb-1">Action Plan</p>
            ...
            <div className="glass-card p-4 space-y-1">
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                <div>
                  ...
                  <p className="text-[10px] text-slate-600">Training Data</p>
                </div>
                <div>
                  ...
                  <p className="text-[10px] text-slate-600">Model R²</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-white">
                    {typeof result.model_info.accuracy_mae === 'number'
                      ? '±' + result.model_info.accuracy_mae.toFixed(1)
                      : result.model_info.accuracy_mae}
                  </p>
...
function RiskCard({ label, score, level, confidence }) {
  const color = score >= 75 ? 'rose' : score >= 55 ? 'orange' : score >= 35 ? 'amber' : 'emerald';
  return (
    <div className="bg-white/5 rounded-xl p-3 text-center space-y-1">
      ...
      {level && (
        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full bg-${color}-500/20 text-${color}-400`}>
          {level}
        </span>
      )}
      {confidence != null && (
        <p className="text-[10px] text-slate-600">{(confidence * 100).toFixed(0)}% conf.</p>
      )}
    </div>
  );
}
...
function HealthScoreCard({ label, data }) {
  const gradeColors = { A: 'emerald', B: 'blue', C: 'amber', D: 'orange', F: 'rose' };
  const c = gradeColors[data.grade] || 'slate';
  return (
    <div className="bg-white/5 rounded-xl p-3 text-center space-y-1">
      <p className="text-[10px] text-slate-500 uppercase tracking-wider">{label}</p>
      <p className="text-2xl font-bold">{data.health_score.toFixed(0)}</p>
      <span className={`text-xs font-bold px-2 py-0.5 rounded-full bg-${c}-500/20 text-${c}-400`}>
        Grade {data.grade}
      </span>
